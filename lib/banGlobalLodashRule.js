...
var Rule = (function (_super) {
    ...
    Rule.FAILURE_STRING = function (lodashFunctionName) {
        return "\"".concat(lodashFunctionName, "\" should be imported from 'lodash-es'");
    };
    return Rule;
...
var BanGlobalLodashWalker = (function (_super) {
    ...
    BanGlobalLodashWalker.prototype.validate = function (node) {
        var _this = this;
        var lodashFunctionNameMatches = node.getText().match(/_\.(\w)*$/g);
        if (lodashFunctionNameMatches) {
            var lodashFunctionNames = lodashFunctionNameMatches.map(function (name) { return name.replace('_.', ''); });
            lodashFunctionNames.forEach(function (lodashFunctionName) {
                _this.addFailureAtNode(node, Rule.FAILURE_STRING(lodashFunctionName));
            });
        }
    };
